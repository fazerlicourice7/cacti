...
def scan_dat(tech_dict, dat_file, tech_flavor, cell_type, temperature):
    ...
    for line in lines:
        if line.startswith("-Vdd"):
            vdd = scan_five_input_double(line, "-Vdd", "V", tech_flavor, print_detail_debug)
            continue
        ...
        if line.startswith("-Wmemcella"):
            ...
            tech_dict['cell_a_w'] = scan_res if scan_res != None else tech_dict.get('cell_a_w')
            continue
        if line.startswith("-Wmemcellpmos"):
            ...
            tech_dict['cell_pmos_w'] = scan_res if scan_res != None else tech_dict.get('cell_pmos_w')
            continue
        if line.startswith("-Wmemcellnmos"):
            ...
            tech_dict['cell_nmos_w'] = scan_res if scan_res != None else tech_dict.get('cell_nmos_w')
            continue
        ...
        if line.startswith("-tsv_pitch"):
            ...
            tech_dict['tsv_liner_dielectric_constant'] = scan_input_double_tsv_type(line, "-tsv_liner_dielectric_cons", "F/um", ic_proj_type, tsv_type, print_detail_debug)
        ...
        if line.startswith("-C_junc_sw"):
            tech_dict['C_junc_sidewall'] = scan_five_input_double(line, "-C_junc_sw", "F/um", tech_flavor, print_detail_debug)
            continue
        ...
        if line.startswith("-n2p_drv_rt"):
            tech_dict['n_to_p_eff_curr_drv_ratio'] = scan_five_input_double(line, "-n2p_drv_rt", "F/um", tech_flavor, print_detail_debug)
            continue
        if line.startswith("-lch_lk_rdc"):
            tech_dict['long_channel_leakage_reduction'] = scan_five_input_double(line, "-lch_lk_rdc", "F/um", tech_flavor, print_detail_debug)
            continue
        ...
        if line.startswith("-wire_pitch"):
            tech_dict['pitch'] = scan_input_double_inter_type(line, "-wire_pitch", "um", ic_proj_type, tech_flavor, print_detail_debug)
            continue
        ...
        if line.startswith("-wire_r_per_micron"):
            tech_dict['R_per_um'] = scan_input_double_inter_type(line, "-wire_r_per_micron", "um", ic_proj_type, tech_flavor, print_detail_debug)
            continue
        if line.startswith("-wire_c_per_micron"):
            tech_dict['C_per_um'] = scan_input_double_inter_type(line, "-wire_c_per_micron", "um", ic_proj_type, tech_flavor, print_detail_debug)
            continue
