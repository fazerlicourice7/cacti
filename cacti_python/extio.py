...
class Extio:
    ...
    def extio_power_term(self):
        self.power_bias = (self.io_param.i_bias * self.io_param.vdd_io +
                           self.io_param.i_leak * (g_ip.num_dq +
                                                   g_ip.num_dqs +
                                                   g_ip.num_clk +
                                                   g_ip.num_ca) * self.io_param.vdd_io / 1e6)

        self.power_termination_read = (1000 * (g_ip.num_dq + g_ip.num_dqs) * self.io_param.vdd_io**2 * 0.25 *
                                       ...
                                       1000 * g_ip.num_ca * self.io_param.vdd_io**2 *
                                       (0.5 / (2 * (self.io_param.r_on_ca + self.io_param.rtt_ca))))

        self.power_termination_write = (1000 * (g_ip.num_dq + g_ip.num_dqs) * self.io_param.vdd_io**2 * 0.25 *
        ...
        if self.io_param.io_type == 'Serial':
            self.power_termination_read = 1000 * g_ip.num_dq * self.io_param.vdd_io * self.io_param.v_sw_clk / self.io_param.r_diff_term
            self.power_termination_write = 1000 * g_ip.num_dq * self.io_param.vdd_io * self.io_param.v_sw_clk / self.io_param.r_diff_term
            self.power_clk_bias = 0

        elif self.io_param.io_type == 'DDR4':
            self.power_termination_read = (1000 * (g_ip.num_dq + g_ip.num_dqs) * self.io_param.vdd_io**2 * 0.5 *
                                           (1 / (self.io_param.r_on + self.io_param.rpar_read + self.io_param.rs1_dq)) +
                                           1000 * g_ip.num_ca * self.io_param.vdd_io**2 *
                                           (0.5 / (2 * (self.io_param.r_on_ca + self.io_param.rtt_ca))))

            self.power_termination_write = (1000 * (g_ip.num_dq + g_ip.num_dqs) * self.io_param.vdd_io**2 * 0.5 *
                                            (1 / (self.io_param.r_on + self.io_param.rpar_write)) +
                                            1000 * g_ip.num_ca * self.io_param.vdd_io**2 *
                                            (0.5 / (2 * (self.io_param.r_on_ca + self.io_param.rtt_ca))))

        if g_ip.iostate == 'READ':
            self.io_power_term = g_ip.duty_cycle * (self.power_termination_read + self.power_bias + self.power_clk_bias)
        elif g_ip.iostate == 'WRITE':
            self.io_power_term = g_ip.duty_cycle * (self.power_termination_write + self.power_bias + self.power_clk_bias)
        elif g_ip.iostate == 'IDLE':
            self.io_power_term = g_ip.duty_cycle * (self.power_termination_write + self.power_bias + self.power_clk_bias)
            if self.io_param.io_type == 'DDR4':
                self.io_power_term = 1e-6 * self.io_param.i_leak * self.io_param.vdd_io
        elif g_ip.iostate == 'SLEEP':
            self.io_power_term = 1e-6 * self.io_param.i_leak * self.io_param.vdd_io
        else:
    ...
    def extio_power_phy(self):
        ...
        if g_ip.iostate == 'READ' or g_ip.iostate == 'WRITE':
            self.phy_power = self.phy_static_power + 2 * self.io_param.frequency * g_ip.num_dq * self.phy_dynamic_power / 1000
        elif g_ip.iostate == 'IDLE':
    ...
    def extio_power_dynamic(self):
        if self.io_param.io_type == 'Serial':
            ...
            self.power_dq_write = (g_ip.num_dq * g_ip.activity_dq *
                                   ...
                                   g_ip.num_dq * g_ip.activity_dq * self.io_param.c_data *
                                   self.io_param.vdd_io * self.io_param.v_sw_data_write_load1 *
                                   self.io_param.frequency / 1000 +
                                   g_ip.num_dq * g_ip.activity_dq * ((g_ip.num_mem_dq - 1) * self.io_param.c_data) *
                                   self.io_param.vdd_io * self.io_param.v_sw_data_write_load2 *
                                   self.io_param.frequency / 1000 +
                                   g_ip.num_dq * g_ip.activity_dq * self.io_param.c_int *
                                   self.io_param.vdd_io * self.io_param.vdd_io * self.io_param.frequency / 1000)

            self.power_dqs_write = (g_ip.num_dqs * (self.io_param.c_tx + c_line) *
                                    self.io_param.vdd_io * self.io_param.v_sw_data_write_line *
                                    self.io_param.frequency / 1000 +
                                    g_ip.num_dqs * self.io_param.c_data * self.io_param.vdd_io *
                                    self.io_param.v_sw_data_write_load1 * self.io_param.frequency / 1000 +
                                    g_ip.num_dqs * ((g_ip.num_mem_dq - 1) * self.io_param.c_data) *
                                    self.io_param.vdd_io * self.io_param.v_sw_data_write_load2 *
                                    self.io_param.frequency / 1000 +
                                    g_ip.num_dqs * self.io_param.c_int * self.io_param.vdd_io *
                                    self.io_param.vdd_io * self.io_param.frequency / 1000)

            self.power_ca_write = (g_ip.num_ca * g_ip.activity_ca *
                                   ...
                                   g_ip.num_ca * g_ip.activity_ca * self.io_param.c_int *
                                   self.io_param.vdd_io * self.io_param.vdd_io * self.io_param.frequency / 1000)

            self.power_dq_read = (g_ip.num_dq * g_ip.activity_dq *
                                  (self.io_param.c_tx + c_line) * self.io_param.vdd_io *
                                  ...
                                  g_ip.num_dq * g_ip.activity_dq * self.io_param.c_int * self.io_param.vdd_io *
                                  self.io_param.vdd_io * self.io_param.frequency / 1000)

            self.power_dqs_read = (g_ip.num_dqs * (self.io_param.c_tx + c_line) *
                                   ...
                                   g_ip.num_dqs * ((g_ip.num_mem_dq - 1) * self.io_param.c_data) *
                                   self.io_param.vdd_io * self.io_param.v_sw_data_read_load2 * self.io_param.frequency / 1000 +
                                   g_ip.num_dqs * self.io_param.c_int * self.io_param.vdd_io * self.io_param.vdd_io *
                                   self.io_param.frequency / 1000)

            self.power_ca_read = (g_ip.num_ca * g_ip.activity_ca *
            ...
            self.power_clk = (g_ip.num_clk *
                              ...
                              g_ip.num_clk * self.io_param.c_int * self.io_param.vdd_io *
                              self.io_param.vdd_io * self.io_param.frequency / 1000)

        if g_ip.iostate == 'READ':
            self.io_power_dynamic = g_ip.duty_cycle * (self.power_dq_read +
                                                       self.power_ca_read + self.power_dqs_read + self.power_clk)
        elif g_ip.iostate == 'WRITE':
            self.io_power_dynamic = g_ip.duty_cycle * (self.power_dq_write + self.power_ca_write + self.power_dqs_write + self.power_clk)
        elif g_ip.iostate == 'IDLE':
            self.io_power_dynamic = g_ip.duty_cycle * self.power_clk
        elif g_ip.iostate == 'SLEEP':
    ...
    def extio_eye(self):
        ...
        if self.io_param.io_type == 'Serial':
            ...
            t_margin_addr_setup = (1e6 * g_ip.addr_timing / (2 * self.io_param.frequency))
            t_margin_addr_hold = (1e6 * g_ip.addr_timing / (2 * self.io_param.frequency))
        else:
            t_margin_write_setup = (1e6 / (4 * self.io_param.frequency)) - \
