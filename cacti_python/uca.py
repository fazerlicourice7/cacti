...
class UCA(Component):
    def __init__(self, dyn_p, g_ip, g_tp):
        ...
        self.area_all_dataramcells = self.bank.mat.subarray.get_total_cell_area() * self.dp.num_subarrays * self.g_ip.nbanks
        ...
        if self.g_ip.is_3d_mem:
            ...
            self.data_bits = self.g_ip.burst_depth * self.g_ip.io_width

            redundancy_perc_TSV = 0.5
            partition_gran = self.g_ip.partition_gran
            if partition_gran == 0:  # Coarse_rank_level
                self.delay_TSV_tot = (self.g_ip.num_die_3d - 1) * tsv_os_bank.delay
                ...
                self.dyn_pow_TSV_tot = self.num_TSV_tot * (self.g_ip.num_die_3d - 1) * tsv_os_bank.power.readOp.dynamic
                self.dyn_pow_TSV_per_access = (self.comm_bits + self.row_add_bits + self.col_add_bits + self.data_bits) * (self.g_ip.num_die_3d - 1) * tsv_os_bank.power.readOp.dynamic
                ...
                self.delay_TSV_tot = self.g_ip.num_die_3d * tsv_os_bank.delay
                ...
                self.area_TSV_tot = self.num_TSV_tot * tsv_os_bank.area.get_area()
                self.dyn_pow_TSV_tot = self.num_TSV_tot * self.g_ip.num_die_3d * tsv_os_bank.power.readOp.dynamic
                self.dyn_pow_TSV_per_access = (self.comm_bits + self.row_add_bits + self.col_add_bits + self.data_bits) * self.g_ip.num_die_3d * tsv_os_bank.power.readOp.dynamic
            elif partition_gran == 2:  # Coarse_bank_level
                self.delay_TSV_tot = self.g_ip.num_die_3d * tsv_os_bank.delay
                self.num_TSV_tot = int((self.comm_bits + self.row_add_bits + self.col_add_bits + self.data_bits / 2) * self.nbanks * self.g_ip.num_tier_row_sprd * self.g_ip.num_tier_col_sprd * (1 + redundancy_perc_TSV))
                self.area_TSV_tot = self.num_TSV_tot * tsv_os_bank.area.get_area()
                self.dyn_pow_TSV_tot = self.num_TSV_tot * self.g_ip.num_die_3d * tsv_os_bank.power.readOp.dynamic
                self.dyn_pow_TSV_per_access = (self.comm_bits + self.row_add_bits + self.col_add_bits + self.data_bits) * self.g_ip.num_die_3d * tsv_os_bank.power.readOp.dynamic
            elif partition_gran == 3:  # Fine_bank_level
                self.delay_TSV_tot = self.g_ip.num_die_3d * tsv_os_bank.delay
                self.num_TSV_tot = int((self.comm_bits + self.row_add_bits + self.col_add_bits + self.data_bits) * self.nbanks * self.g_ip.ndwl * self.g_ip.ndbl / self.g_ip.num_tier_col_sprd / self.g_ip.num_tier_row_sprd * (1 + redundancy_perc_TSV))
                self.area_TSV_tot = self.num_TSV_tot * tsv_os_bank.area.get_area()
                self.dyn_pow_TSV_tot = self.num_TSV_tot * self.g_ip.num_die_3d * tsv_os_bank.power.readOp.dynamic
                self.dyn_pow_TSV_per_access = (self.comm_bits + self.row_add_bits + self.col_add_bits + self.data_bits) * self.g_ip.num_die_3d * tsv_os_bank.power.readOp.dynamic
            else:
            ...
            self.area_lwl_drv = self.membus_RAS.area_lwl_drv * self.g_ip.nbanks
            self.area_row_predec_dec = self.membus_RAS.area_row_predec_dec * self.g_ip.nbanks
            self.area_col_predec_dec = self.membus_CAS.area_col_predec_dec * self.g_ip.nbanks

            self.area_subarray = self.membus_RAS.area_subarray * self.g_ip.nbanks
            self.area_bus = self.membus_RAS.area_bus * self.g_ip.nbanks

            self.area_data_drv = self.membus_data.area_data_drv * self.g_ip.nbanks
            self.area_IOSA = self.membus_data.area_IOSA * self.g_ip.nbanks
            self.area_sense_amp = self.membus_data.area_sense_amp * self.g_ip.nbanks

            self.area_address_bus = self.membus_RAS.area_address_bus * (1.0 + self.comm_bits / (self.row_add_bits + self.col_add_bits)) * self.g_ip.nbanks
            self.area_data_bus = self.membus_RAS.area_data_bus + self.membus_data.area_local_dataline * self.g_ip.nbanks
            ...
            self.area_per_bank = (self.area_lwl_drv + self.area_row_predec_dec + self.area_col_predec_dec +
                                  ...
                                  self.area_address_bus + self.area_data_bus) / self.g_ip.nbanks + self.area_sense_amp
            ...
            if self.g_ip.num_die_3d > 1 or self.g_ip.partition_gran > 0:
                self.total_area_per_die = self.area_all_dataramcells + self.area_TSV_tot
            else:
                self.total_area_per_die = self.area_all_dataramcells

            if self.g_ip.is_3d_mem and self.g_ip.print_detail_debug:
                print("-------  CACTI 3D DRAM Main Memory -------")
                print("\nMemory Parameters:\n")
                print(f"    Total memory size (Gb): {int(self.g_ip.cache_sz)}")
                print(f"    Number of banks: {int(self.g_ip.nbanks)}")
                print(f"    Technology size (nm): {self.g_ip.F_sz_nm}")
                print(f"    Page size (bits): {self.g_ip.page_sz_bits}")
                print(f"    Burst depth: {self.g_ip.burst_depth}")
                print(f"    Chip IO width: {self.g_ip.io_width}")
    ...
    def compute_delays(self, inrisetime):
        ...
        if self.g_ip.is_3d_mem:
            ...
            self.delay_before_subarray_output_driver = parameter.symbolic_convex_max(max_delay_before_row_decoder + delay_inside_mat, 
                                                                           (delay_array_to_mat + self.bank.mat.b_mux_predec.delay + 
                                                                           self.bank.mat.bit_mux_dec.delay + self.bank.mat.delay_sa))
            tmp_max = parameter.symbolic_convex_max(self.delay_array_to_sa_mux_lev_1_decoder, self.delay_array_to_sa_mux_lev_2_decoder)
            self.delay_before_subarray_output_driver = parameter.symbolic_convex_max(self.delay_before_subarray_output_driver, tmp_max)
            ...
            if self.dp.is_main_mem:
                ...
                # CHANGE: MAX
                cas_latency = parameter.symbolic_convex_max(self.delay_array_to_sa_mux_lev_1_decoder, self.delay_array_to_sa_mux_lev_2_decoder) + self.delay_from_subarray_out_drv_to_out
            ...
            self.g_ip.rpters_in_htree = True
            if self.g_ip.rpters_in_htree == False:
                temp = parameter.symbolic_convex_max(temp, self.bank.htree_in_add.max_unpipelined_link_delay)
            ...
            self.multisubbank_interleave_cycle_time = parameter.symbolic_convex_max(delay_req_network, delay_rep_network)
    ...
    def compute_power_energy(self):
        ...
        if self.g_ip.is_3d_mem:
            ...
            self.write_energy = (
                ...
                self.bank.mat.power_subarray_out_drv.readOp.dynamic +
                self.membus_data.power.readOp.dynamic +
                self.bank.mat.power_sa.readOp.dynamic * self.g_ip.burst_depth * self.g_ip.io_width / self.g_ip.page_sz_bits +
                datapath_energy
            )
            ...
            col_cycle_act_row = (1e-6 / self.g_ip.sys_freq_MHz) / 2 * self.g_ip.burst_depth
            ...
            if self.g_ip.print_detail_debug:
                ...
                print(f"uca.cc: membus_data->power.readOp.dynamic = {self.membus_data.power.readOp.dynamic * 1e9} nJ")
                print(f"uca.cc: bank.mat.power_sa = {self.bank.mat.power_sa.readOp.dynamic * self.g_ip.burst_depth * self.g_ip.io_width / self.g_ip.page_sz_bits * 1e9} nJ")
