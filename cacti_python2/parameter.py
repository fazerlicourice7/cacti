...
class InputParameter:
    ...
    def error_checking(self): 
        ...
        if (self.fully_assoc or self.pure_cam) and (
                self.data_arr_ram_cell_tech_type == ram_cell_tech_type_num.lp_dram or self.data_arr_ram_cell_tech_type == ram_cell_tech_type_num.comm_dram):
...
class TechnologyParameter:
    ...
    def init(self, g_ip, technology, is_tag):
        ...
        dram_acc_lo.assign(g_ip, self, in_file_lo, ram_cell_tech_type if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram else dram_cell_tech_flavor, g_ip.temp)
        dram_acc_hi.assign(g_ip, self, in_file_hi, ram_cell_tech_type if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram else dram_cell_tech_flavor, g_ip.temp)
        self.dram_acc.interpolate(alpha, dram_acc_lo, dram_acc_hi)
        if tech_lo <= 22:
            pass
        elif tech_lo <= 32:
            self.dram_acc.Vth = 0.44129 if ram_cell_tech_type == ram_cell_tech_type_num.lp_dram else 1.0
        elif tech_lo <= 45:
            self.dram_acc.Vth = 0.44559 if ram_cell_tech_type == ram_cell_tech_type_num.lp_dram else 1.0
        elif tech_lo <= 65:
            self.dram_acc.Vth = 0.43806 if ram_cell_tech_type == ram_cell_tech_type_num.lp_dram else 1.0
        elif tech_lo <= 90:
            self.dram_acc.Vth = 0.4545 if ram_cell_tech_type == ram_cell_tech_type_num.lp_dram else 1.0
        ...
        dram_wl_lo.assign(g_ip, self, in_file_lo, ram_cell_tech_type if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram else dram_cell_tech_flavor, g_ip.temp)
        dram_wl_hi.assign(g_ip, self, in_file_hi, ram_cell_tech_type if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram else dram_cell_tech_flavor, g_ip.temp)
        ...
        if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram:
        ...
        wire_local_lo.assign(in_file_lo, g_ip.ic_proj_type, 3 if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram else 0)
        wire_local_hi.assign(in_file_hi, g_ip.ic_proj_type, 3 if ram_cell_tech_type == ram_cell_tech_type_num.comm_dram else 0)
...
class DynamicParameter:
    def __init__(self, g_ip: InputParameter, g_tp: TechnologyParameter, is_tag_=False, pure_ram_=0, pure_cam_=0, Nspd_=1.0, Ndwl_=1, Ndbl_=1, Ndcm_=1, Ndsam_lev_1_=1, Ndsam_lev_2_=1, wt="Global", is_main_mem_=False):
        self.g_ip = g_ip
        self.g_tp = g_tp
    ...
    def init_parameters(self):
        ...
        self.is_dram = (self.ram_cell_tech_type == ram_cell_tech_type_num.lp_dram or self.ram_cell_tech_type == ram_cell_tech_type_num.comm_dram)
        ...
        if self.is_dram:
            ...
            if self.ram_cell_tech_type == ram_cell_tech_type_num.comm_dram:
